...
int bitCount(int x) {
  ...
  x = (x & m1) + ((x >> 1) & m1) +
      ((x >> 2) & m1) + ((x >> 3) & m1);
  x = x + (x >> 4);
  x = (x & lowNibbles) + ((x >> 16) & lowNibbles);
  x = x + (x >> 8);
  return x & 0x3f;
}
...
int satSub(int x, int y) {
  int result = x + ~y + 1;
  int overflow = ((x ^ y) & (x ^ result)) >> 31;
  int saturated = (result >> 31) ^ (1 << 31);

  return result ^ (overflow & (result ^ saturated));
}
...
int float_f2i(unsigned uf) {
  ...
  unsigned value = (uf & 0x7fffff) | 0x800000;

  if (exponent < 127)
    return 0;
  if (exponent > 157)
    return 0x80000000u;
  value = (value << 7) >> (157 - exponent);
  if (uf >> 31)
    return -value;
  return value;
}
...
unsigned float_greater(unsigned x, unsigned y) {
  unsigned ax = x & 0x7fffffff;
  unsigned ay = y & 0x7fffffff;
  unsigned sx = x >> 31;
  unsigned sy = y >> 31;
